...
class RankingQuestionViewController: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    func getDoubt() {
        ...
        orderedQuestions = auxQuestion.sort({ $0.upvotes > $1.upvotes })
    }
    ...
    func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell {
        ...
        if question.my_vote == 0 {
            cell.likeButton.addTarget(self, action: #selector(RankingQuestionViewController.likeButtonPressed), forControlEvents: .TouchUpInside)
            cell.likeButton.setImage(ImageUtil.imageLikeButton, forState: .Normal)
            cell.likeButton.tintColor = UIColor.grayColor()
        } else {
            cell.likeButton.addTarget(self, action: #selector(RankingQuestionViewController.deleteLikeButtonPressed), forControlEvents: .TouchUpInside)
        }
