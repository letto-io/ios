...
class AttachmentDoubtResponseViewController: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    var doubt = Doubt()
    var attachmentResponse = Array<Contributions>()
    ...
    func getDoubtResponse() {
        ...
        for i in 0 ..< doubtContributions.count {
            ...
            if doubtContributions[i].mcmaterial.mime.containsString(StringUtil.image) ||
                doubtContributions[i].mcmaterial.mime.containsString(StringUtil.applicationPdf) {
                auxContributions.insert(doubtContributions[i], atIndex: j)
                j += 1
            }
        }
        
        attachmentResponse = auxContributions
    }
    
    func tableView(tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        return attachmentResponse.count
    }
    ...
    func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell {
        ...
        let doubtResponse = attachmentResponse[ indexPath.row ]
